            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="container">
                        ...
                        <form action="{{ route('bencana.update', $bencana->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')

                            <div class="mb-3">
                                <label for="nama_bencana" class="form-label">Nama Bencana:</label>
                                <input type="text" class="form-control" id="nama_bencana" name="nama_bencana" value="{{ old('nama_bencana', $bencana->nama_bencana) }}" required>
                            </div>

                            <div class="mb-3">
                                <label for="lokasi_bencana" class="form-label">Lokasi Bencana:</label>
                                <input type="text" class="form-control" id="lokasi_bencana" name="lokasi_bencana" value="{{ old('lokasi_bencana', $bencana->lokasi_bencana) }}" required>
                            </div>

                            <div class="mb-3">
                                <label for="waktu_kejadian" class="form-label">Waktu Kejadian:</label>
                                <input type="datetime-local" class="form-control" id="waktu_kejadian" name="waktu_kejadian" value="{{ old('waktu_kejadian', $bencana->waktu_kejadian ? $bencana->waktu_kejadian->format('Y-m-d\TH:i') : null) }}" required>
                            </div>

                            <div class="mb-3">
                                <label for="deskripsi" class="form-label">Deskripsi (Opsional):</label>
                                <textarea class="form-control" id="deskripsi" name="deskripsi" rows="3">{{ old('deskripsi', $bencana->deskripsi) }}</textarea>
                            </div>

                            <div class="mb-3">
                                <label for="jumlah_korban" class="form-label">Jumlah Korban (Opsional):</label>
                                <input type="number" class="form-control" id="jumlah_korban" name="jumlah_korban" value="{{ old('jumlah_korban', $bencana->jumlah_korban) }}" min="0">
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                @if ($bencana->foto_bencana)
                                    <img src="{{ asset('storage/' . $bencana->foto_bencana) }}" alt="Foto Bencana" class="mt-2 img-thumbnail" width="200">
                                @endif
                            </div>
