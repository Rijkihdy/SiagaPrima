    ...
    <div class="py-6">
        <div class="mx-auto px-6">
            <div class="bg-white overflow-hidden shadow-md sm:rounded-lg">
                <div class="p-6">
                    <h1 class="text-2xl font-bold mb-6">Tambah Data Bencana</h1>
                    ...
                    <!-- Form -->
                    <form action="{{ route('bencana.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf

                        <div>
                            <label for="nama_bencana" class="block font-medium text-gray-700">Nama Bencana</label>
                            <input type="text" name="nama_bencana" id="nama_bencana" value="{{ old('nama_bencana') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500" required>
                        </div>

                        <div>
                            <label for="lokasi_bencana" class="block font-medium text-gray-700">Lokasi Bencana</label>
                            <input type="text" name="lokasi_bencana" id="lokasi_bencana" value="{{ old('lokasi_bencana') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500" required>
                        </div>

                        <div>
                            <label for="waktu_kejadian" class="block font-medium text-gray-700">Waktu Kejadian</label>
                            <input type="datetime-local" name="waktu_kejadian" id="waktu_kejadian" value="{{ old('waktu_kejadian') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500" required>
                        </div>

                        <div>
                            <label for="deskripsi" class="block font-medium text-gray-700">Deskripsi (Opsional)</label>
                            <textarea name="deskripsi" id="deskripsi" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500">{{ old('deskripsi') }}</textarea>
                        </div>

                        <div>
                            <label for="jumlah_korban" class="block font-medium text-gray-700">Jumlah Korban (Opsional)</label>
                            <input type="number" name="jumlah_korban" id="jumlah_korban" value="{{ old('jumlah_korban') }}" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500">
                        </div>
